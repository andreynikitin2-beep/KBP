...
async function main() {
  ...
  await db.insert(schema.emailTemplates).values([
    ...
    { key: "report_error", label: "Сообщение об ошибке в инструкции", subject: "Ошибка в инструкции «{{title}}» — сообщение от {{reporter}}", body: "Уважаемый(ая) {{owner}},\n\nПользователь {{reporter}} сообщил об ошибке в инструкции «{{title}}».\n\nТекст сообщения:\n{{message}}\n\nСсылка на материал: {{link}}", description: "Отправляется автору и администратору, когда пользователь сообщает об ошибке в материале. Переменные: {{title}}, {{reporter}}, {{owner}}, {{message}}, {{link}}" },
    { key: "suggest_improvement", label: "Предложение по улучшению", subject: "Предложение по улучшению «{{title}}» от {{reporter}}", body: "Уважаемый(ая) {{owner}},\n\nПользователь {{reporter}} предлагает улучшение для инструкции «{{title}}».\n\nПредложение:\n{{message}}\n\nСсылка на материал: {{link}}", description: "Отправляется автору и администратору, когда пользователь предлагает улучшение материала. Переменные: {{title}}, {{reporter}}, {{owner}}, {{message}}, {{link}}" },
  ]);
