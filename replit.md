...
-   **Approval Workflow**: Sequential two-step approval chain. (1) Author submits draft via "Запустить согласование". (2a) If author == material owner/deputy → only section owner approval needed. (2b) Otherwise → material owner/deputy approves first, then section owner. Any approver can "Отклонить публикацию" (requires comment) — rejection permanently blocks resubmission of that draft; author must create a new version. Admin can force-publish bypassing all steps. `approvalStep` field on MaterialVersion tracks current step ("material_owner" | "section_owner"). `rejectedAt` flags permanently rejected drafts. `getSectionOwnerIds` / `isSectionOwner` helpers in kbLogic.ts resolve section owners via catalogNodes hierarchy.
...
-   **AD/SSO Integration & User Management**: Extends user profiles with AD synchronization capabilities (`adAccountName`, `lastSyncAt`, `deactivatedAt`). Provides admin tools for AD/SSO configuration (supports SAML, OIDC, LDAP protocols), user deactivation/reactivation, and local user creation (with password field). LDAP mode includes Bind DN, Bind Password, and Base DN fields for service account configuration. Deactivation triggers material status changes for owners/deputies.
